...
module.exports = class AccessUtils {
  ...
  attachAccessObserver(modelName) {
    ...
    Model.observe('access', (ctx, next) => {
      ...
      // this.warnContextValues('ACCESS observer');
      ...
      if (currentUser) {
        ...
        this.buildFilter(currentUser.getId(), ctx.Model)
          .then(filter => {
    ...
    Model.observe('before save', (ctx, next) => {
      ...
      if (currentUser) {
        ...
        if (foreignKey && currentUser[foreignKey]) {
          ...
          // TODO: To work around the getUserContext sometimes storing wrong currentUser only set this if it's not explicitly set
          if (!foreignKeySet) {
            ctx.instance[foreignKey] = currentUser[foreignKey];
            logger.debug(`BEFORE SAVE observer - Updated object ${util.inspect(ctx.instance)}`);
          } else {
            logger.debug(`BEFORE SAVE observer - Foreign key already set ${util.inspect(ctx.instance)}`);
          }
        } else {
   ...
   */
  getCurrentUser() {
    this.debugContextValues('getCurrentUser');
  ...
  getCurrentUserGroups() {
    this.debugContextValues('getCurrentUserGroups');
  ...
  debugContextValues(callee) {
    const ctx = loopback.getCurrentContext();

    logger.debug(`${callee} - req.accessToken ${ctx && ctx.get('req.accessToken') && ctx.get('req.accessToken').id}`);
    logger.debug(`${callee}  - req.originalUrl ${ctx && ctx.get('req.originalUrl')}`);
    logger.debug(`${callee}  - ctx.get ${ctx && util.inspect(ctx.get('currentUserGroups'))}`);
    logger.debug(`${callee}  - ctx.accessToken ${ctx && ctx.get('accessToken') && ctx.get('accessToken').id}`);
  }

  warnContextValues(callee) {
    const ctx = loopback.getCurrentContext();

    logger.warn(`${callee} - req.accessToken ${ctx && ctx.get('req.accessToken') && ctx.get('req.accessToken').id}`);
    logger.warn(`${callee}  - req.originalUrl ${ctx && ctx.get('req.originalUrl')}`);
    logger.warn(`${callee}  - ctx.get ${ctx && util.inspect(ctx.get('currentUserGroups'))}`);
    logger.warn(`${callee}  - ctx.accessToken ${ctx && ctx.get('accessToken') && ctx.get('accessToken').id}`);
  }
        ...
        .spread((currentGroupCount, targetGroupCount) => {
          ...
          // Note the fact that we are allowing access due to passing an ACL.
           if (bAllowAccess) {
            ...
            // const loopbackContext = this.app.loopback.getCurrentContext();
            const loopbackContext = loopback.getCurrentContext();
            loopbackContext.set('groupAccessApplied', true);
          }
