...
module.exports = class AccessUtils {
  ...
  getCurrentUser() {
    ...
    // logger.debug(`getCurrentUser - ctx ${util.inspect(ctx)}`);
    // logger.debug(`getCurrentUser - ctx.active ${ctx && util.inspect(ctx.active)}`);
    logger.debug(`getCurrentUser - ctx.active ${ctx && ctx.active && ctx.active.currentUser && util.inspect(ctx.active.currentUser.id)}`);
    logger.debug(`getCurrentUser - ctx.get ${ctx && util.inspect(ctx.get('currentUser') && util.inspect(ctx.get('currentUser').id))}`);
    logger.debug(`getCurrentUser - ctx.accessToken ${ctx && util.inspect(ctx.get('accessToken'))}`);
  ...
  getCurrentUserGroups() {
    const ctx = this.app.loopback.getCurrentContext();
    // logger.debug(`getCurrentUserGroups - ctx ${util.inspect(ctx)}`);
    // logger.debug(`getCurrentUserGroups - ctx.active ${util.inspect(ctx)}`);
    logger.debug(`getCurrentUserGroups - ctx.active ${ctx && ctx.active && util.inspect(ctx.active.currentUserGroups)}`);
    logger.debug(`getCurrentUserGroups - ctx.get ${ctx && util.inspect(ctx.get('currentUserGroups'))}`);
    logger.debug(`getCurrentUserGroups - ctx.accessToken ${ctx && util.inspect(ctx.get('accessToken'))}`);
