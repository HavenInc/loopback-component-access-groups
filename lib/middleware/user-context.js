...
module.exports = function userContextMiddleware() {
  ...
  return function userContext(req, res, next) {
    ...
    logger.debug(`user-context - accessToken ${loopbackContext.get('accessToken')}`);
    logger.debug(`user-context - currentUser ${loopbackContext.get('currentUser')}`);
    logger.debug(`user-context - currentUserGroups ${loopbackContext.get('currentUserGroups')}`);
