...
module.exports = function accessLoggerMiddleware() {
  ...
  return function accessLogger(req, res, next) {
    ...
    res.on('finish', () => {
      ...
      logger.debug(`${chalk.cyan('Finish Request')} - accessToken ${req.req.accessToken} ${req.method} ${req.originalUrl} lbHttpMethod: ${req.method} lbUrl ${req.originalUrl} lbStatusCode ${res.statusCode} lbResponseTime  ${duration} lbResponseTimeUnit: 'ms'`);
    });
