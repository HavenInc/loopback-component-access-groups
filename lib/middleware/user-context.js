...
module.exports = function userContextMiddleware() {
  ...
  return function userContext(req, res, next) {
    ...
    winston.debug(`user-context - current access token ${loopbackContext.get('accessToken')}`);
    winston.debug(`user-context - current user ${loopbackContext.get('currentUser')}`);
    winston.debug(`user-context - current user groups ${loopbackContext.get('currentUserGroups')}`);
