...
module.exports = function userContextMiddleware() {
  ...
  return function userContext(req, res, next) {
    ...
    logger.debug(`access-logger - accessToken ${token && util.inspect(token)}`);
    logger.debug(`access-logger - currentUser ${user && user.id}`);
    logger.debug(`access-logger - currentUserGroups ${group && util.inspect(group)}`);
