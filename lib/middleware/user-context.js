...
module.exports = function userContextMiddleware() {
  ...
  return function userContext(req, res, next) {
    ...
    logger.debug(`user-context - current access token ${loopbackContext.get('accessToken')}`);
    logger.debug(`user-context - current user ${loopbackContext.get('currentUser')}`);
    logger.debug(`user-context - current user groups ${loopbackContext.get('currentUserGroups')}`);
