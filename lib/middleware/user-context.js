...
module.exports = function userContextMiddleware() {
  ...
  return function userContext(req, res, next) {
    ...
    logger.debug(`user-context - reqToken ${reqToken && reqToken.id}`);
    logger.debug(`user-context - reqUrl ${reqUrl && reqUrl.id}`);
    ...
    logger.debug(`user-context - ctx ${loopbackContext && util.inspect(loopbackContext)}`);
    ...
    return Promise.join(
      ...
      (user, groups) => {
        ...
        loopbackContext.set('req.accessToken', req.accessToken);
        loopbackContext.set('req.originalUrl', req.originalUrl);
