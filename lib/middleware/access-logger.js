...
module.exports = function accessLoggerMiddleware() {
  ...
  return function accessLogger(req, res, next) {
    ...
    res.on('finish', () => {
      ...
      let url = loopbackContext.get('req.originalUrl');
      let token = loopbackContext.get('accessToken');
      let user = loopbackContext.get('currentUser');
      let group = loopbackContext.get('currentUserGroups');

      logger.debug(`access-logger - finish - reqToken ${reqToken && reqToken.id}`);
      logger.debug(`access-logger - finish - url ${url && util.inspect(url)}`);
      logger.debug(`access-logger - finish - accessToken ${token && token.id}`);
